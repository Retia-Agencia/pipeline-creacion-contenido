...
| D12 | **El refactor saca de Apify todo gasto que no sea una corrida.** El título y la miniatura de un video tienen que salir por otra vía, **sin pagar**. Solo se le paga al proveedor de scraping por buscar | Mani, 25/09 | Medido el 25/09: Colecciones (`apps/dashboard/lib/apify.ts`: `traerMetadata` de ADR-072 y `traerVideoUrls`) gastó **~2,2 USD** del 16 al 25/09, contra ~2,5 del motor, del mismo cupo y sin pre-flight ([costos.md §1.1](../costos.md)). Encaja con la dirección del mismo día: el cockpit pasa a tracker y el scraping se terceriza (cierre 159). ⚠️ **Sin vía elegida todavía**: pide enmendar ADR-072, y la descarga del mp4 necesita una URL firmada que hoy solo da Apify |
